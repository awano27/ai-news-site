...
def main() -> int:
    ...
    for en in idx:
        ...
        # Date-named snapshots carry a top-level "date"; *_daily.json
        # snapshots carry metadata.source_date instead (and some carry
        # neither — their index date is derived from article published_at,
        # which we don't re-derive here).
        candidates = {
            data.get('date'),
            (data.get('metadata') or {}).get('source_date'),
        } - {None, ''}
        if candidates and d not in candidates:
            print('[mismatch] top-level date mismatch:', jf, sorted(candidates), '!=', d)
            ok = False
